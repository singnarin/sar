...
$date = date("Y-m-d");
header('Content-Disposition: attachment; filename="สรุปผลภาพรวมการศึกษาขั้นพื้นฐานของสถานศึกษา'.$date.'.xls"');
?>
...
<?php
	if ($_SESSION['ses_username']!=""){	
		...
		$selb1 = mysql_query("SELECT * FROM `b1` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		$b1num = mysql_num_rows($selb1);
		if ($b1num > 0) {
			$b1Result = mysql_fetch_array($selb1) or die  (mysql_error());
		}
		$selb2 = mysql_query("SELECT * FROM `b2` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		$b2num = mysql_num_rows($selb2);
		if ($b2num > 0) {
			$b2Result = mysql_fetch_array($selb2) or die  (mysql_error());
		}
		$selb3 = mysql_query("SELECT * FROM `b3` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		...
		$selb5 = mysql_query("SELECT * FROM `b5` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		$b5num = mysql_num_rows($selb5);
		if ($b5num > 0) {
			$b5Result = mysql_fetch_array($selb5) or die  (mysql_error());
		}
		$selb6 = mysql_query("SELECT * FROM `b6` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		...
		$selb8 = mysql_query("SELECT * FROM `b8` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		...
		$selb10 = mysql_query("SELECT * FROM `b10` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		...
		$selb13 = mysql_query("SELECT * FROM `b13` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		$b13num = mysql_num_rows($selb13);
		if ($b13num > 0) {
			$b13Result = mysql_fetch_array($selb13) or die  (mysql_error());
		}
		$selb14 = mysql_query("SELECT * FROM `b14` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
		$b14num = mysql_num_rows($selb14);
		if ($b14num > 0) {
			$b14Result = mysql_fetch_array($selb14) or die  (mysql_error());
		}
		$selb15 = mysql_query("SELECT * FROM `b15` WHERE `schoolid` = '".$_SESSION['ses_username']."'");
...
?>
	<table align="center" width="100%" border="1" >
			...
			<tr>
				...
				<td><div align="center"><?php echo $teacherResult["primaryteacher"];?></div></td>
